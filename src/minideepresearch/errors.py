...
def validate_llm_response(response_content: str, model_name: str, min_length: int = 10) -> None:
    """
    ...
    if len(response_content) < min_length:
        ...
        raise ValueError(f"LLM returned very short response: {len(response_content)} chars")
    
    # Check for common issues
    if "I don't know" in response_content.lower() or "cannot answer" in response_content.lower():
...
def validate_questions(questions: list[str], expected_count: int) -> None:
    ...
    invalid = [q for q in questions if not q.strip() or len(q) < 5]
    if invalid:
